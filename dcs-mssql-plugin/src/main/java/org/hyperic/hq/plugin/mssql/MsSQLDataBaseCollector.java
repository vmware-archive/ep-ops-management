...
public class MsSQLDataBaseCollector extends MsSQLCollector {
    ...
    public void collect() {
        ...
        if (properties.containsKey("v")) {
            collectDBFreePercentage(properties);
        } else {
            super.collect();
        }
    }

    private void collectDBFreePercentage(Properties properties) {
    ...
    public static List<List<String>> executeSqlCommand(List<String> scriptPropertiesList) throws PluginException {
        ...
        for (String command : scriptPropertiesList) {
            if (command.contains("\"")) {
                throw new PluginException ("Invalid SQL command");
            }
        }
    ...
    public static List<String> prepareSqlCommand(Properties props) {
        ...
        addEncodingConfigCmd(scriptPropertiesList);
    ...
    /**
     * In order to support high-ascii characters, we need to:
     * 1- Configure the encoding to utf-8 prior to calling sqlcmd or osql.
     * This is done by changing the Active Code Page to UTF-8 (represented by 65001).
     * 2- Decode the output in UTF-8. This is done in the runCommand call.
     */
    private static void addEncodingConfigCmd(List<String> scriptPropertiesList) {
        scriptPropertiesList.add("cmd.exe");
        scriptPropertiesList.add("/c");
        scriptPropertiesList.add("chcp");
        scriptPropertiesList.add("65001");
        scriptPropertiesList.add(">NUL");
        scriptPropertiesList.add("&&");
    }
    ...
    public static String runCommand(String[] command, int timeout) throws PluginException {
        ...
        try {
            ...
            throw new PluginException("Fail decoding command response");
        }
        MsSQLDetector.debug(log, "out: " + out);
        MsSQLDetector.debug(log, "ExitValue: " + exec.getExitValue());
        if (exec.getExitValue() != 0) {
            throw new PluginException("Command execution terminated with erronous Exit Value: " + out);
        }
        return out;
    }
}