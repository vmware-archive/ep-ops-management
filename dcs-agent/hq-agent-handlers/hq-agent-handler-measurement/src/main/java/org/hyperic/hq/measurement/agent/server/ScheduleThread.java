...
/**
 * The schedule thread which maintains the schedule, and dispatches on them. After data is retrieved, it is sent to the
 * SenderThread which handles depositing the results on disk, and sending them to the bizapp.
 */

public class ScheduleThread extends AgentMonitorSimple implements Runnable, AgentDiagnosticObject {
    private static final String SCHEDULE_THREAD_METRICS_COLLECTED_TIME =
                AgentStatsCollector.SCHEDULE_THREAD_METRICS_COLLECTED_TIME;
    private static final String SCHEDULE_THREAD_METRIC_TASKS_SUBMITTED =
                AgentStatsCollector.SCHEDULE_THREAD_METRIC_TASKS_SUBMITTED;
    private static final String SCHEDULE_THREAD_METRIC_COLLECT_FAILED =
                AgentStatsCollector.SCHEDULE_THREAD_METRIC_COLLECT_FAILED;
    ...
    private static final long MINUTE_INTERVAL = 60000; // 1 minute
    ...
    private final TLongHashSet scheduled = new TLongHashSet();
    ...
    ScheduleThread(Sender sender,
        ...
        if (disableDiag)
            log.info("Disabling ScheduleThread Diagnostics");
        this.sender = sender;
    ...
    /**
     * The MetricVerificationTask iterates over the list of FutureTasks that have been submitted for execution. Each
     * task is checked for completion and then removed. For tasks that do not complete within the timeout are cancelled,
     * which will attempt to free up the executor running the task. NOTE: This will only work if the hung task is in an
     * interrupt-able state i.e. sleep() or wait()
     */
    ...
    private static class ResourceSchedule {
        ...
        private final TLongObjectHashMap<Object> collected = new TLongObjectHashMap<Object>();
    }
    ...
    boolean isMeasurementsScheduledForResource(AppdefEntityID ent) {
        synchronized (schedules) {
            return schedules.containsKey(ent.getAppdefKey());
        }
    }
    ...
    void scheduleMeasurement(ScheduledMeasurement meas) {
        ...
        try {
            ...
            long offset = schedulerOffsetManager.getSchedluerOffsetForInterval(interval);
            rs.schedule.scheduleItem(meas, interval, offset, true, true);
            if (log.isDebugEnabled()) {
                Long timeOfNext;
                try {
    ...
    private void logCache(String basicMsg,
                          ParsedTemplate tmpl,
                          String msg,
                          Exception exc,
                          boolean printStack) {
    ...
    /**
     * A method which does the main logging for the run() method. It ensures that we don't perform excessive logging
     * when plugins generate a lot of errors.
     * 
    ...
    private class MetricTask implements Runnable {
        ...
        MetricTask(ResourceSchedule rs,
                   ScheduledMeasurement meas) {
            this.rs = rs;
            this.meas = meas;
        }
        ...
        /**
         * @return 0 if task is still queued for execution, otherwise the amount of time in milliseconds the task has
         *         been running.
         */
        ...
        public void run() {
            ...
            try {
                ...
                long now = now();
                long nowRounded = TimingVoodoo.roundDownTime(now, MINUTE_INTERVAL);
                ...
                data = getMeasurementPluginManager().getValue(dsn.plugin, dsn.metric);

                if (meas.getUnits().equals(MeasurementConstants.UNITS_PERCENTAGE) &&
                            !meas.getCategory().equals(MeasurementConstants.CAT_AVAILABILITY)) {
                    data.multiplyValue(100);
                }
                ...
                long interval = meas.getInterval();
                long offset = schedulerOffsetManager.getSchedluerOffsetForInterval(interval);
                long time = TimingVoodoo.roundDownTime(now, interval, offset);
            ...
            if (success) {
                ...
                sender.processData(meas.getDsnID(), data, meas.getInterval(), meas.getDerivedID(),
    ...
    private MeasurementPluginManager getMeasurementPluginManager()
        throws AgentRunningException, PluginException {
        return AgentDaemon.getMainInstance().getMeasurementPluginManager();
    }

    private void setDiagInfo(MetricValue data,
                             ParsedTemplate dsn,
                             ResourceSchedule rs,
                             long mid) {
        if (disableDiag) {
    ...
    private void setDiagScheduled(ResourceSchedule rs,
                                  boolean incrementSchedules) {
        if (disableDiag) {
            return;
        }
    ...
    private void collect(ResourceSchedule rs,
                         List<ScheduledMeasurement> items) {
        ...
        for (int i = 0; (i < items.size()) && (!shouldDie.get()); i++) {
            ...
            synchronized (executors) {
                try {
                    GenericPlugin p = getMeasurementPluginManager().getPlugin(tmpl.plugin).getProductPlugin();
                    plugin = p.getName();
    ...
    /**
     * The main loop of the ScheduleThread, which watches the schedule waits the appropriate time, and executes
    ...
    private class MetricValuePlusId {
        ...
        private MetricValuePlusId(long mid,
                                  MetricValue value) {
            this.mid = mid;
            this.value = value;
        }
