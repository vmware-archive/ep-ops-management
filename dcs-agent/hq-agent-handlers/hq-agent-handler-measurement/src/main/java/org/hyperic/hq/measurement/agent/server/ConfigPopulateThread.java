...
/**
 * This class is used to query the HQ server for the configurations that HQ knows about (for entities which the agent
 * should be servicing). It retries until it can get the configurations, after which it will exit. Really only useful
 * when the agent started up.
 * 
 * This class will only return those entities that have been configured for log or config file tracking.
 */
            ...
            extends Thread
{
    ...
    private final Log log = LogFactory.getLog(ConfigPopulateThread.class.getName());
            ...
            try {
                ...
                if (LogTrackPlugin.isEnabled(config, id.getType())) {
                    this.log.info("Creating log track plugin " +
                                ents[i].getPluginName());
                    AgentDaemon.getMainInstance().getLogTrackPluginManager().createPlugin(ents[i]
                                .getPluginName(), ents[i].getPluginType(), config);
                }
            } catch (Exception exc) {
