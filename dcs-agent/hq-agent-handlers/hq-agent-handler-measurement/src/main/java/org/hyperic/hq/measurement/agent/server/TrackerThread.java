...
class TrackerThread implements Runnable {
    ...
    private final Object interrupter;
    ...
    TrackerThread(AgentStorageProvider storage,
        ...
        throws AgentStartException
    {
        ...
        Properties bootProperties = bootConfig.getBootProperties();
        String info = bootProperties.getProperty(CONFIGTRACK_LISTNAME);
        if (info != null) {
            storage.addOverloadedInfo(CONFIGTRACK_LISTNAME, info);
        }
        info = bootProperties.getProperty(LOGTRACK_LISTNAME);
        if (info != null) {
        ...
        String sMaxBatchSize = bootProperties.getProperty(PROP_MAXEVENTBATCHSIZE);
    ...
    private void flushEvents(LinkedList events,
                             String dListName)
    {
        if (events.isEmpty()) {
            return;
    ...
    {
        LinkedList events = getLogTrackPluginManager().getEvents();
     ...
     */
    private boolean processNextEventReport(String dListName,
                                           int batchSize) {
    ...
    private void removeProcessedEventsFromStorage(String dListName,
        ...
        for (Iterator i = this.storage.getListIterator(dListName); numEventsProcessed > 0 && i != null && i.hasNext(); numEventsProcessed--) {
            i.next();
            i.remove();
        }
    }
    ...
    private LogTrackPluginManager getLogTrackPluginManager() {
        return AgentDaemon.getMainInstance().getLogTrackPluginManager();
    }
}
