...
public class SenderThread extends AgentMonitorSimple implements Sender, Runnable {
    ...
    private static final int PROP_RECSIZE = 68; // 68 byte records.
    ...
    SenderThread(AgentConfig bootConfig,
        ...
        this.client = setupClient(bootConfig);
    ...
    private MeasurementCallbackClient setupClient(AgentConfig bootConfig)
        throws AgentStartException
    {
        StorageProviderFetcher fetcher;

        fetcher = new StorageProviderFetcher(this.storage);
        return new MeasurementCallbackClient(fetcher, bootConfig);
    }
    ...
    private static class Record {
        ...
        public boolean equals(Object rhs) {
            ...
            if (rhs instanceof Record) {
                Record r = (Record) rhs;
                return ((r.derivedID == derivedID) && (r.data.getTimestamp() == data.getTimestamp()));
            }
            return false;
        }
    ...
    /**
     * This routine moves all the data from the transition queue into the storage provider, so it can be shipped to the
     * server.
     */
        ...
        synchronized (this.transitionQueue) {
            for (Record rec : this.transitionQueue) {
                try {
                    ...
                    if (rec.isAvail) {
                        this.storage.addToList(AVAILABILITY_LISTNAME,
                                    encodedRec);
                    } else {
                        this.storage
                                    .addToList(MEASURENENT_LISTNAME, encodedRec);
                    }
                } catch (Exception exc) {
    ...
    /**
     * Send a batch of measurement points back to the server. This method sends at most maxBatchSize elements back.
     * 
     * @return null if all the measurements in the list have been sent, otherwise it returns the timestamp of the last
    ...
    private SendBatchResult sendBatch(String listName,
        ...
        for (Iterator<Record> recordsIt = records.iterator(); recordsIt.hasNext(); num++) {
            Record rec = recordsIt.next();
    ...
    private void syncToServerTime() {
        ...
        try {
            ...
            modifyServerTimeDiff(agentTime, agentTime + fetchStoredServerTimeDiff());
        }
    }
     ...
     */
    public synchronized void syncToServerTime(long agentTime,
                                              long serverTime) {
    ...
    private void modifyServerTimeDiff(long agentTime,
                                      long serverTime) {
    ...
    private int removeMeasurements(int num,
        ...
        for (Iterator i = this.storage.getListIterator(listName); (i != null) && i.hasNext() && (j < num); j++) {
            i.next();
            i.remove();
        }
    ...
    public void run() {
        ...
        while (this.shouldDie == false) {
            try {
                ...
                // We are sending 2 different batches, the first is for
                // availability and the second
                // is for measurement. We are doing this because we want to make
                // sure that availability
                // data will get processed by the server even if the server is
                // not able to process measurement
