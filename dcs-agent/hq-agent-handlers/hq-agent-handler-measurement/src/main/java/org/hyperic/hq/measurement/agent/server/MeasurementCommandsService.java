 ...
 */
public class MeasurementCommandsService implements MeasurementCommandsClient, ShouldShutdownGracefully {
    ...
    private static final int[] resourcesType = { TypeInfo.TYPE_PLATFORM, TypeInfo.TYPE_SERVER, TypeInfo.TYPE_SERVICE };
    ...
    /**
     * @see org.hyperic.hq.measurement.agent.client.MeasurementCommandsClient#addTrackPlugin(java.lang.String,
    ...
    public void addTrackPlugin(String id,
        ...
        if (pluginType.equals(ProductPlugin.TYPE_LOG_TRACK)) {
            manager = AgentDaemon.getMainInstance().getLogTrackPluginManager();
        }
        else {
    ...
    void addTrackPlugin(TrackPluginAdd_args ta)
        throws AgentRemoteException {
    ...
    public GetMeasurements_result getMeasurements(GetMeasurements_args args)
        ...
        for (i = 0; i < nArgs; i++) {
            ...
            try {
                ...
                val = AgentDaemon.getMainInstance().getMeasurementPluginManager().getValue(arg);
    ...
    /**
     * @see org.hyperic.hq.measurement.agent.client.MeasurementCommandsClient#removeTrackPlugin(java.lang.String,
     *      java.lang.String)
     */
    ...
    void removeTrackPlugin(TrackPluginRemove_args ta)
        throws AgentRemoteException {
        String id = ta.getName();
        String pluginType = ta.getType();
    ...
    private class Scheduler extends Thread {
        ...
        public void run() {
            try {
                // if we we asked graceful shutdown, wait till queue is empty before going down.
                while (!shutdown.get() || (isGracefully && !argQueue.isEmpty())) {
                    try {
                        final List<ScheduleMeasurements_args> list = new ArrayList<ScheduleMeasurements_args>();
                        argQueue.drainTo(list);
                        if (!list.isEmpty()) {
                            _log.info("Going to schedule measurements for " + list.size() + " resources.");
                ...
                _log.info("Scheduler has exit from the run loop.");
            } catch (Throwable t) {
                _log.error(t, t);
            }
        }
    ...
    private void scheduleMeasurements(Collection<ScheduleMeasurements_args> args)
        ...
        ListIterator<ScheduledMeasurement> metricsListIterator = metrics.listIterator(metrics.size());
        while (metricsListIterator.hasPrevious()) {
            _scheduleObject.scheduleMeasurement(metricsListIterator.previous());
        }
        if (debug) {
    ...
    /**
     * Unschedule the measurements of a resource given its internal id by trying to figure out what the resource's type
     * is. LWO will be unscheduled for servers. Virtual servers will be unscheduled for platforms.
     */
    public void unscheduleMeasurementsById(UnscheduleMeasurementsById_args args)
        throws AgentRemoteException {
        int resourceInternalId = args.getResourceInternalId();
        _log.debug("Processing UnscheduleMeasurementsById command for resource id " + resourceInternalId);
        ...
        for (int resourceType : resourcesType) {
            ...
            if (_scheduleObject.isMeasurementsScheduledForResource(id)) {
                _log.debug("Unscheduling measurements for resource " + resourceType + ":" + resourceInternalId);
                deleteMeasurements(id);
                mearuementsFound = true;
            }
        }
        if (!mearuementsFound) {
            _log.debug("No measurements were found to unschedule for resource id " + resourceInternalId);
        }
    }

    /**
     * Unschedule the measurements of a resource given its AppdefEntityID
     * 
     * @see org.hyperic.hq.measurement.agent.client.MeasurementCommandsClient#unscheduleMeasurementsById(org.hyperic.hq.measurement.agent.commands.UnscheduleMeasurements_args)
     */
    public void unscheduleMeasurements(UnscheduleMeasurements_args args)
        throws AgentRemoteException {
        int i, nEnts = args.getNumEntities();
        _log.debug("Received unschedule request for " + nEnts + " resources");

        for (i = 0; i < nEnts; i++) {
            AppdefEntityID ent = args.getEntity(i);
            deleteMeasurements(ent);
        }
    }

    private void deleteMeasurements(AppdefEntityID ent)
        throws AgentRemoteException {
    ...
    public void shutdownGracefully() {
        ...
        // busy wait till we finish handling the queue
        while (_scheduler.isAlive()) {
            try {
                _log.info("Waiting for the scheduler to finish proccessing the queue...");
                Thread.sleep(20000);
            } catch (InterruptedException e) {
            }
        }
