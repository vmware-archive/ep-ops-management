...
/**
 * A callback class that notifies the starting entity (most likely the agent client) as to the startup status of the
...
public class AgentStartupCallback {
     ...
     * @throws IOException if the callback fails to establish a connection to the starting entity.
     */
    ...
    private void connectToPort(int sPort)
        throws IOException {

        final int minute = 60000;
        int nextAttemptGap = (int) (0.5 * minute); // start with half a minute
        int maxAttemptGap = 30 * minute; // wait no more then half an hour
        IOException exceptionToThrow = null;

        while (nextAttemptGap <= maxAttemptGap) {
            try {
                _startupSock = new Socket("localhost", sPort);
                return;
            ...
            try {
                _log.info("Waiting " + ((double) nextAttemptGap / minute)
                            + " minutes before retrying connection to Server port.");
     ...
     * 
     * @param succeeded <code>true</code> if the agent startup succeeded; <code>false</code> if the agent startup
     *            failed.
     */
