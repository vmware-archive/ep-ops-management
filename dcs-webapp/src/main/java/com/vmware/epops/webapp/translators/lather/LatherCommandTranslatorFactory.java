...
public enum LatherCommandTranslatorFactory {
    INSTANCE; // a single-element enum type is the best way to implement a
    // singleton according to Effective Java 2ed.

    private final Map<String, LatherCommandTranslator> translatorsMap = new HashMap<String, LatherCommandTranslator>();

    public void registerTranslator(String commandName,
                                   LatherCommandTranslator translator) {
        translatorsMap.put(commandName, translator);
    }

    public LatherCommandTranslator getTranslator(String commandName) {
