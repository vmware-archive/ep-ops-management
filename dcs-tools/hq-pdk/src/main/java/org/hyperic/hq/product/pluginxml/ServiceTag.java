...
class ServiceTag
    ...
    public void endAttributes()
        ...
        String modelString = getAttribute(ATTR_MODEL);
        ObjectModel model = ObjectModel.STANDARD;
        if (modelString != null) {
            model = ObjectModel.valueOf(modelString.toUpperCase());
            if (model == null) {
                String msg = "Invalid model" + "=\"" + modelString + "\"";
                throw new XmlAttrException(msg);
            }
        }
        ...
        if (this.server != null) {
            ...
            service =
                        this.server.addService(name, isInternal, model);
            if (description != null) {
