    ...
    public ConfigOption getConfigOption() {
        switch (this) {
            case ENABLE_TOPN_COLLECTION:
                return new BooleanConfigOption(this.name, "Enable top processes collection", false);

            case TOPN_COLLECTION_INTERVAL_IN_MINUTES:
                ConfigOption intervalOption =
                            new ConfigOption(this.name, "Top processes collection interval in minutes",
                                        "1") {
                                    ...
                                    try {
                                        ...
                                        if (val <= 0) {
                                            throw new InvalidOptionValueException(
                                                        "top_processes.interval must be larger than 0");
                                        }
