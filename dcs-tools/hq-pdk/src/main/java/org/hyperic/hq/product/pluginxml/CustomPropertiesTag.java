...
public class CustomPropertiesTag extends ConfigTag {

    private static final String[] OPTIONAL_ATTRS = {
                ATTR_NAME, ATTR_INCLUDE
    };

    CustomPropertiesTag(BaseTag parent) {
        super(parent);
    }
    ...
    protected ConfigOptionTag getConfigOptionTag() {
        return new CustomPropertyTag(this);
    }

    protected void addConfigSchema(String typeName,
                                   int idx,
                                   ConfigSchema schema) {
        this.data.addCustomPropertiesSchema(typeName, schema);
    }
}
