...
/**
 * A CompositeRuntimeResourceReport is sent from an agent to the CAM server to report resources detected at runtime
 * through monitoring capabilities. Resources detected in a runtime scan are automatically added to the appdef model,
 * and do not require explicit approval the way that platforms and servers detected through regular autoinventory scans
 * do. <br>
 * <br>
 * A CompositeRuntimeResourceReport is essentially a Map. The keys in the map are Integers representing the serverIds of
 * the servers that performed the runtime scan. The values are RuntimeResourceReport objects representing the platforms,
 * servers, and services that were detected in the scan for that server. <br>
 * <br>
 */
public class CompositeRuntimeResourceReport {
    ...
    /**
     * This method will be used by AXIS in SOAP deserialization. Generally you do not want to call this method directly,
     * instead use the addServerReport method.
     */
    ...
    /**
     * Add a RuntimeResourceReport for the specified server. Any existing report for the server within this composite
    ...
    public String simpleSummary() {
        ...
        for (int i = 0; i < _serverReports.length; i++) {
            ...
            for (int j = 0; j < platforms.length; j++) {
                ...
                AIServerValue[] servers = ((AIPlatformValue) platforms[j]).getAIServerValues();
