            ...
            implements AutoServerDetector
{
    ...
    /**
     * First checks if the ptql query is specified in the process properties, if not, then it checks to see if the port
     * value is specified, to generate the service URL.
     * 
    ...
    /**
     * Goes through several checks in order to find the jmx url in the configuration. For each process argument it will
     * check the following: 1) Does it use the ptql query option to detect the process (i.e
     * -Dcom.sun.management.jmxremote=ptql:State.Name...). 2) Does it match -Dcom.sun.management.jmxremote.port=<port>.
     * 3) Did the user configure the jmx.url in the ui(optionally the jmx.username and jmx.password). 4) Does it match
     * the ptql query generated using the install path. 5) Otherwise it returns false.
     * 
     * @param config
     * @param args
     * @param processQuery
     ...
     */
    protected boolean findAndSetURL(ConfigResponse config,
                                    List<String> args,
                                    String processQuery) {
    ...
    private boolean configureUserSpecifiedMxUrl(ConfigResponse config,
                                                String arg,
    ...
    /**
     * Sets the JMX url. First checks whether the process supplies the jmx.url. Then does some searching for the url.
     * 
     * @param process
     * @param config
     * @throws MxRuntimeException If there is no jmx.url found.
     */
    protected void setJmxUrl(MxProcess process,
                             ConfigResponse config) {
        boolean urlSet = false;
    ...
    protected List discoverMxServices(MBeanServerConnection mServer,
        ...
        for (int i = 0; i < queries.size(); i++) {
            MxObjectNameQuery query = (MxObjectNameQuery) queries.get(i);
            ...
            if (name == null) {
                name = extractServiceNameFromQuery(query);
            }
    ...
    /**
     * Extract service name from the given query
     */
    private String extractServiceNameFromQuery(MxObjectNameQuery query) {
        String name = "";
        String queryName = query.getName(includeNoneKeyPropertiesInServiceName());
    ...
    /**
     * A MBeans name consists of two parts, the domain and the key properties. See:
     * http://docs.oracle.com/javase/7/docs/api/javax/management/ObjectName.html The key properties can have a "none"
     * value in case no value was discovered.
     * 
     * @return true in order to include the "none" properties in the name of the services discovered by the detector,
     *         false otherwise
     */
    ...
    public Set discoverServiceTypes(ConfigResponse serverConfig)
        ...
        try {
            ...
            serviceTypes =
                        serviceTypeFactory.create(getProductPlugin(), (ServerTypeInfo) getTypeInfo(), mServer,
                                    objectNames);
        } catch (Exception e) {
            throw new PluginException(e.getMessage(), e);
        } finally {
