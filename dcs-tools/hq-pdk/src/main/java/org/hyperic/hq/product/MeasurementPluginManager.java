...
public class MeasurementPluginManager extends PluginManager implements MeasurementValueGetter {
    ...
    private final Map metricCache = Collections.synchronizedMap(new HashMap());
    private boolean debugRateMetrics;
    private static final ArrayList<Double> DOWN_VALUES = new ArrayList<Double>(Arrays.asList(Metric.AVAIL_DOWN,
                Metric.AVAIL_PAUSED, Metric.AVAIL_POWERED_OFF, Metric.AVAIL_WARN));
    private static final ArrayList<Double> UP_VALUES = new ArrayList<Double>(Arrays.asList(Metric.AVAIL_UP, 1.0));
    ...
    private void registerProxy(String name,
    ...
    private MetricValue getPluginValue(String name,
                                       Metric metric)
        throws PluginException,
        ...
        try {
            MetricValue metricVal = invoker.getValue(metric);

            if (Metric.ATTR_AVAIL.equalsIgnoreCase(metric.getAttributeName())) {
                convertAvailabilityValues(metricVal);
            }
            if (log.isTraceEnabled()) {
                log.trace("Got metric value from plugin '" + name + "': attributeName: '" + metric.getAttributeName()
                            + "' categoryName: '" + metric.getCategory()
                            + "' value: '" + metricVal.getValue()
                            + "' ID: '" + metric.getId()
                            + "' domainName: '" + metric.getDomainName()
                            + "' interval: '" + metric.getInterval()
                            + "' (" + metric.toDebugString() + ")");
            }
    ...
    // Here we convert the Availability metric value of old plug-ins from 1 to 100
    // We override 0.5, -0.01 & -0.02 to be 0 and ignore any other value except 0 & 100
    private void convertAvailabilityValues(MetricValue metricVal) {

        double metricNumeralValue = metricVal.getValue();
        if (UP_VALUES.contains(metricNumeralValue)) {
            metricVal.setValue(Metric.AVAIL_UP);
        } else if (DOWN_VALUES.contains(metricNumeralValue)) {
            metricVal.setValue(Metric.AVAIL_DOWN);
        } else { // UNKNOWN
            metricVal.setValue(MetricValue.VALUE_NONE);
        }
    }
    ...
    public String getHelp(TypeInfo info,
        ...
        String trackHelp =
                    TrackEventPluginManager.getGenericHelp(ppm.getLogTrackPluginManager(), schema, info);
        if (trackHelp != null) {
            trackHelp =
    ...
    public void reportEvent(Metric metric,
        ...
        if (metric.getId() != null) {
            ProductPluginManager ppm = (ProductPluginManager) getParent();
            plugin = ppm.getLogTrackPluginManager().getLogTrackPlugin(metric.getId());
        }
