...
/**
 * A utility class to facilitate comparisons of AI value objects. This is used in determining if we should send a
 * default scan or a runtime scan to the server. If nothing has changed, we don't send the report.
 * 
...
public class AICompare {
    ...
    public static boolean measurementInfosEqual(byte[] m1,
        ...
        // skip length comparison here - measurement infos may or may not have an attributes map, but could be equal in
        // all ways that count
        ...
        try {
            ...
            for (Iterator iterator = measurementNames.iterator(); iterator.hasNext();) {
                ...
                boolean measurementsEqual =
                            compare(mi1.getMeasurement(measurementName), mi2.getMeasurement(measurementName));
    ...
    public static boolean configSchemasEqual(byte[] c1,
        ...
        try {
            ...
            for (int i = 0; i < optionNames.length; i++) {
                ...
                if (!(compare(option1.getName(), option2.getName()) || compare(option1.getDescription(),
    ...
    public static ConfigDiff configsDiff(byte[] newConf,
                                         byte[] oldConf)
     ...
     * 
     * @return true if both strings are null, or if they are both non-null and ".equals" returns true. return false
     *         otherwise
     */
    private static boolean compare(String s1,
                                   String s2) {
        if (s1 == s2) {
            return true;
        }
    ...
    private static boolean compare(MeasurementInfo m1,
                                   MeasurementInfo m2) {
        if (m1 == m2) {
            return true;
        }
        ...
        return compare(m1.getAlias(), m2.getAlias()) && compare(m1.getCategory(), m2.getCategory()) &&
                    compare(m1.getGroup(), m2.getGroup()) && compare(m1.getName(), m2.getName())
                    && compare(m1.getRate(), m2.getRate()) &&
                    compare(m1.getTemplate(), m2.getTemplate()) && compare(m1.getUnits(), m2.getUnits())
                    && (m1.getCollectionType() == m2.getCollectionType()) &&
                    (m1.getInterval() == m2.getInterval()) && (m1.isDefaultOn() == m2.isDefaultOn())
                    && (m1.isIndicator() == m2.isIndicator());
    }
     ...
     * @return
     */
    private static boolean compare(String[] strArray1,
                                   String[] strArray2) {
