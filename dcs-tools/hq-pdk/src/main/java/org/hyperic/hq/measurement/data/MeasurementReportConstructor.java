...
public class MeasurementReportConstructor {
    /**
     * clientIDs is a hash onto a hash of dsn IDs onto lists of ValueData objects
     */
    ...
    public void addDataPoint(long clientID,
                             long dsnID,
                             MetricValue data)
    {
        HashMap dsnMap;
        ArrayList valData;
        Long iClientID, iDsnID;
    ...
    public DSNList[] constructDSNList() {
        ...
        for (Iterator i = this.clientIDs.entrySet().iterator(); i.hasNext();) {
            ...
            for (Iterator j = dsnMap.entrySet().iterator(); j.hasNext();) {
                ...
                dsns[dsnIdx].setDsnId(dsnID.longValue());
                dsns[dsnIdx].setValues(data);
                dsnIdx++;
            }

            cids[cidIdx] = new DSNList();
            cids[cidIdx].setClientId(clientId.longValue());
