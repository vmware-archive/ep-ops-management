...
/**
 * The server signature described HOW to detect whether a server exists on a platform, without providing any details
 * about how to discover anything about it beyond the fact that it exists. Useful when the agent is trying to discover
 * what it has installed on its platform to decide which plugins it will need to pull from the server. Before the agent
 * has any plugins it can use this class to determine which ones it will need.
 */
