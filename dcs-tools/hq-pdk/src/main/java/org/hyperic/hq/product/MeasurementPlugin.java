...
public class MeasurementPlugin extends GenericPlugin {
    ...
    /**
     * Allow xml template properties to be added by a plugin. The properties can be used as if the hq-plugin.xml defined
     * &lt;property name="..." value="..."/&gt; for each entry in the Map returned. Properties with the same name in the
     * xml file will override these.
     */
    ...
    private void setAvailInterval(TypeInfo type,
    ...
    /**
     * If the MeasurementInfo collection type is "trendsup" and rate attribute is not set to "none" then create a new
     * metric: - name changed to original name plus the rate. - alias changed to original name plus the rate. -
     * collection type is changed to "dynamic" - turn off designate for the original metric. - append __RATE__=$rate to
     * the measurement template.
     */
    private static MeasurementInfo generateRateMetric(MeasurementInfo info) {

        if (info.getCollectionType() != MeasurementConstants.COLL_TYPE_TRENDSUP)
        {
            return null;
        }
    ...
    protected String getPluginXMLHelp(TypeInfo info,
                                      String name,
                                      Map props) {
        if (this.data == null) {
            log.debug(getName() + " has no PluginData");
    ...
    /**
     * This method is called when the plugin is asked for a metric value. The Metric is a translated value as returned
     * by the getMeasurements() routine, and then run through the translate() method.
     * 
     * @param metric Value returned from translate(), representing a specific metric to retrieve
     * 
     * @return The value of the Metric and timestamp of collection time
     * 
     * @throws MetricInvalidException The plugin is unable to use the metric, generally a developer bug where the
     *             template is malformed. I.e. JMX MalformedObjectNameException
     * 
     * @throws MetricNotFoundException The monitored resource does not know about the requested Metric. I.e. JMX
     *             AttributeNotFoundException
     * 
     * @throws MetricUnreachableException The monitored resource is unreachable. I.e. ConnectException
     * 
    ...
    /**
     * Translate a measurement as returned from getMeasurements() into a value which can be passed into the plugin's
     * getValue() routine.
     * 
     * @param template Measurement template from one of the plugins measurements returned from getMeasurements()
     * @param config Configuration used to perform translation on the template
     * 
     * @throws PluginException When an internal plugin error occurs
     * @throws MetricInvalidException When the template passed cannot be mapped to a template returned via
     *             getMeasurements()
     */
