...
public interface RegistryServerDetector {
     ...
     * @return A List of ServerResource objects representing the servers that were found in the registry entry. It is
     *         possible for multiple servers to be in a single entry, although it is unusual. If the registry entry
     *         being scanned is the CurrentControlSet, then only a single server can be found in the entry, although the
     *         AI code does not enforce this requirement. This method should return null if no servers were found.
