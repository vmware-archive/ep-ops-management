...
/**
 * This implementation of ScanMethod knows how to scan the filesystem. It can be configured to scan the whole
 * filesystem, or to only scan certain directories while ignoring others. It can also ignore entire filesystems of a
 * particular type, for example network mounted filesystems or CD-ROM drives (ie iso9660 filesystems).
 */
public class FileScan
    ...
    public static final String[] FS_TYPES = { FS_TYPE_ALL, FS_TYPE_LOCAL, FS_TYPE_NETWORK };
    ...
    public void scan(ConfigResponse platformConfig,
        ...
        try {
            ...
            _followSymlinks = Boolean.valueOf(_config.getValue("followSymlinks")).booleanValue();
            ...
            while (matchedServerTypes.hasNext()) {
                serverDetector = (ServerDetector) matchedServerTypes.next();
                stMatches = (List) matchResults.matches.get(serverDetector);
                for (i = 0; i < stMatches.size(); i++) {
                    matchPath = (String) stMatches.get(i);
                    prevDetector = (ServerDetector) pathsToDetectors.get(matchPath);
                    if (prevDetector != null) {
                        throw new AutoinventoryException("Multiple servers ("
    ...
    /**
     * Convert our concept of which filesystems to scan into one of the FS_XXX constants defined in MatcherConfig
     */
    ...
    private static final String[] WIN32_DEFAULT_EXCLUDE_DIRS = { "\\WINNT", "\\TEMP", "\\TMP",
                "\\Documents and Settings", "\\Recycled" };

    private static final String[] UNIX_DEFAULT_DIRS = { "/usr", "/opt" };
    private static final String[] UNIX_DEFAULT_EXCLUDE_DIRS = { "/usr/doc", "/usr/dict", "/usr/lib", "/usr/libexec",
    ...
    private String getDefaultScanDirs() {
        ...
        if (_isWindows) {
            ...
            for (i = 0; i < WIN32_DEFAULT_DIRS.length; i++) {
                if (dirs.length() > 0)
                    dirs += DirArrayConfigOption.DELIM_STR;
