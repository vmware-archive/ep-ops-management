 ...
 * 
 * This class supports adding one server, and its child services. This class also supports adding multiple versions of
 * the same server.
 * 
 * Typically, the plugin developer uses the default <code>PLATFORM_NAMES</code> for the list of platforms supported by a
 * given server. The plugin developer can specify her own list of supported platforms.
 * 
 * @since 1.0
 * @version 1.0
 */
public class TypeBuilder {
     ...
     */
    public TypeBuilder(String serverName,
                       String serverDesc) {
     ...
     * @return ServerTypeInfo
     */
    public ServerTypeInfo addServer(String name,
                                    String version) {
    ...
    /**
     * This method adds a list of services to the this.entity list. All the services added through this method will be
     * marked with the internal flag set to false
     * 
    ...
    public void addServices(ServerTypeInfo server,
                            String[] services) {
        addServices(server, services, ObjectModel.STANDARD);
    }
    ...
    public void addServices(ServerTypeInfo server,
        ...
        for (int i = 0; i < services.length; i++) {
            addService(server, services[i], model);
        }
    }
    ...
    public ServiceTypeInfo addService(ServerTypeInfo server,
                                      String name) {
        return addService(server, name, ObjectModel.STANDARD);
    }
    ...
    /**
     * Given a serverName and a serviceName, return the name of the service type. This procedure is abstracted out into
     * this method because it needs to be reused in runtime discovery.
     */
    ...
    /**
     * Add a ServerTypeInfo clone of the given server, changing valid platforms to the given platforms. Clone all
     * services from the given server.
     * 
