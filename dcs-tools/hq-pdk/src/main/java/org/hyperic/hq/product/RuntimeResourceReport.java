...
/**
 * A RuntimeResourceReport represents the results of a single server's runtime scan. It is comprised of an array of
 * platforms. The platforms are matched against appdef by FQDN. Within each platform, the servers are matched against
 * appdef by ID (only if it is the same as the reporting server), and then by autoinventory-identifier. Within each
 * server, the services are matched against appdef by name.
 */
