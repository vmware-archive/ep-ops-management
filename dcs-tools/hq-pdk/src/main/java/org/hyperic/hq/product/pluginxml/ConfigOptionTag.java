...
class ConfigOptionTag extends ContainerTag {
    ...
    static final String ATTR_ADVANCED = "advanced";
    ...
    private static final String[] OPTIONAL_ATTRS = {
                ATTR_DEFAULT, ATTR_TYPE, ATTR_OPTIONAL, ATTR_CATEGORY, ATTR_PLATFORM, ATTR_READONLY, ATTR_ADVANCED
    };
    ...
    void endTag()
        ...
        if (type.equals("string")) {
            ...
            ConfigSchema.addSecret(name);
            option = this.schema.addSecret(name, desc);
        }
        else if (type.equals("sensitive")) {
            ConfigSchema.addSensitive(name);
            option = this.schema.addSensitive(name, desc);
        }
        ...
        if ("true".equals(getAttribute(ATTR_READONLY))) {
            option.setReadonly(true);
        }

        String advanced = getAttribute(ATTR_ADVANCED);
        if (advanced != null) {
            option.setAdvanced("true".equals(advanced) ? Boolean.TRUE : Boolean.FALSE);
        }

        option.setCategory(getAttribute(ATTR_CATEGORY));

        option.setPlatform(getAttribute(ATTR_PLATFORM));
