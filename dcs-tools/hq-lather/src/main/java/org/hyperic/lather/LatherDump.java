...
/**
 * A utility class (which must reside in the same package as the LatherValue class) which dumps the contents of a
 * LatherValue object to a stream.
 */
public class LatherDump {
    ...
    private void dumpEnt(TextIndenter tInd,
                         Map map,
                         String type) {
    ...
    private void dumpList(TextIndenter tInd,
                          Map map,
                          String type) {
        for (Iterator i = map.entrySet().iterator(); i.hasNext();) {
            Map.Entry ent = (Map.Entry) i.next();
    ...
    private void dump(LatherValue val,
                      TextIndenter tInd) {
        ...
        for (Iterator i = val.getObjectVals().entrySet().iterator(); i.hasNext();)
        {
            Map.Entry ent = (Map.Entry) i.next();
        ...
        for (Iterator i = val.getObjectLists().entrySet().iterator(); i.hasNext();)
        {
            Map.Entry ent = (Map.Entry) i.next();
            List l = (List) ent.getValue();
