 ...
 * 
 * LatherValue objects have a variety of ways to interact with them. The most simplistic way is to use simple key/value
 * pairs.
 * 
 * The object also supports keys which map onto lists of data.
 * 
 * Values contained within the object can have associated primitive types, such as int, double, byte[] and String.
 * 
 * Access to LatherValue is unsynchronized, so callers must do their own.
 */
public abstract class LatherValue {
    ...
    private Map longLists; // Strings -> List of Longs
    private final Map<String, Serializable> serializableMap = new HashMap<String, Serializable>();

    private boolean ensureOrder;
    public static String INVALID_VALUE_ERROR = "Invalid Value";

    /**
     * Get a map used for storing the different types of values (i.e. setStringVals, etc.)
     */
    ...
    private void setup(boolean ensureOrder) {
        ...
        this.longLists = this.createStorageMap();
    }
    ...
    /**
     * Using this constructor guarantees that encoding the exact same LatherValue 2 different times generates the same
     * byte order, as the maps are traversed in the same order.
     */
    ...
    protected void setStringValue(String key,
                                  String value) {
        this.checkArg(value);
    ...
    protected void setIntValue(String key,
                               int value) {
    ...
    protected void setDoubleValue(String key,
                                  double value) {
    ...
    protected void setLongValue(String key,
                                long value) {
    ...
    protected void setObjectValue(String key,
                                  LatherValue value) {
        this.checkArg(value);
    ...
    protected void setByteAValue(String key,
                                 byte[] value) {
        this.checkArg(value);
        this.byteaVals.put(key, value);
    }

    private List getListValueForAdd(Map map,
                                    String listName) {
    ...
    private List getListValueForGet(Map map,
                                    String listName) {
        List res = (List) map.get(listName);
    ...
    protected void addStringToList(String listName,
                                   String value) {
    ...
    protected void addIntToList(String listName,
                                int value) {
    ...
    protected void addDoubleToList(String listName,
                                   double value) {
    ...
    protected void addLongToList(String listName,
                                 long value) {
        List list = this.getListValueForAdd(this.longLists, listName);

        list.add(new Long(value));
    }

    protected long[] getLongList(String listName) {
        List list = this.getListValueForGet(this.longLists, listName);
        long[] res;
        int idx;
        ...
        for (Iterator i = list.iterator(); i.hasNext();) {
            ...
            res[idx++] = val.longValue();
        }

        return res;
    }

    protected void addByteAToList(String listName,
                                  byte[] value) {
    ...
    protected void addObjectToList(String listName,
                                   Object value) {
    ...
    protected void addObject(String objectName,
                             Serializable object) {
    ...
    /**
     * This method is called to verify that a LatherValue object has all the appropriate contents. It is invoked after
     * the LatherXCoder decodes a stream and formulates a LatherValue.
     */
