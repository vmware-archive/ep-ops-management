 ...
 * </p>
 * <p>
 * Conversions can be applied to the <code>n</code>th argument after the format in the argument list, rather than to the
 * next unused argument. In this case, the conversion characer % is replaced by the sequence %<code>n</code>$, where
 * <code>n</code> is a decimal integer giving the position of the argument in the argument list.
 * </p>
 * <p>
 * In format strings containing the %<code>n</code>$ form of conversion specifications, each argument in the argument
 ...
 * </p>
 * <p>
 * In format strings containing the %<code>n</code>$ form of a conversion specification, a field width or precision may
 * be indicated by the sequence *<code>m</code>$, where m is a decimal integer giving the position in the argument list
 * (after the format argument) of an integer argument containing the field width or precision.
 * </p>
 * <p>
 * The format can contain either numbered argument specifications (that is, %<code>n</code>$ and *<code>m</code>$), or
 * unnumbered argument specifications (that is % and *), but normally not both. The only exception to this is that %%
 * can be mixed with the %<code>n</code>$ form. The results of mixing numbered and unnumbered argument specifications in
 ...
 * specified).</td></tr>
 * <dt>+
 * <dd>result of a signed conversion always begins with a sign (+ or -). (It will begin with a sign only when a negative
 ...
 *          fix (2 digits in e form corrupt) rounding in f format implemented round up when digit not printed is 5
 *          formatting of -0.0f round up/down when last digits are 50000...
 */
public class PrintfFormat {
    ...
    /**
     * Return a substring starting at <code>start</code> and ending at either the end of the String <code>s</code>, the
     * next unpaired percent sign, or at the end of the String if the last character is a percent sign.
     * 
