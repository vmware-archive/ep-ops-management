...
/**
 * Implements a generic pager. What is a pager? Let's say you have a large collection of objects. You're interested in
 * breaking the mammoth list out into a number pages, each with 25 items on it. You're interested in returning page #17
 * of such a collection. Why bother implementing the "skip past a bunch of things, then return pagesize items in the
 * resultant colleciton" over and over again.
 * 
 * You can also have the elements go through a _processor that you supply as they move from the source collection to the
...
public class Pager {
    ...
    /**
     * Seek to the specified pagenum in the source collection and return pagsize numberof of elements in the List. If
     * pagenum or pagesize is -1, then everything in the source collection will be returned.
     * 
     * @param source The source collection to seek through.
     * @param pagenum The page number to seek to. If there not enough pages in the collection, then an empty list will
     *            be returned.
     * @param pagesize The size of each page.
     * @return PageList containing results of seek.
     */
    public PageList seek(Collection source,
                         int pagenum,
                         int pagesize) {
    ...
    /**
     * Seek to the specified pagenum in the source collection and return pagsize numberof of elements in the List, as
     * specified the PageControl object. If pagenum or pagesize is -1, then everything in the source collection will be
     * returned.
     * 
     * @param source The source collection to seek through.
    ...
    /**
     * Seek to the specified pagenum in the source collection and place pagesize number of elements into the dest
     * collection. If pagenum or pagesize is -1, then everything in the source collection will be placed in the dest
     * collection.
     * 
     * @param source The source collection to seek through.
     ...
     * @param pagesize The size of each page.
     */
    public void seek(Collection source,
                     Collection dest,
                     int pagenum,
                     int pagesize) {
     ...
     */
    public int seek(Collection source,
                    Collection dest,
                    int pagenum,
                    int pagesize,
                    Object procData) {
        ...
        for (i = 0, currentPage = 0; iter.hasNext() && currentPage < pagenum; i++, currentPage +=
                    (i % pagesize == 0) ? 1
                                : 0) {
            iter.next();
        }
    ...
    /**
     * Seek to the specified pagenum in the source collection and place pagesize number of elements into the dest
     * collection. Unlike, seek(), all items are passed to the Processor or ProcessorExt regardless whether they are
     * placed in dest collection. If pagenum or pagesize is -1, then everything in the source collection will be placed
     * in the dest collection.
     * 
     * @param source The source collection to seek through.
     * @param pagenum The page number to seek to. If there not enough pages in the collection, then an empty list will
     *            be returned.
     ...
     * @param procData any object required to process the item.
     */
    public PageList seekAll(Collection source,
                            int pagenum,
                            int pagesize,
                            Object procData) {
        PageList dest = new PageList();
     ...
     * @param procData any object required to process the item.
     */
    public int seekAll(Collection source,
                       Collection dest,
                       int pagenum,
                       int pagesize,
                       Object procData) {
        ...
        for (i = 0, currentPage = 0; iter.hasNext() && currentPage < pagenum; currentPage +=
                    (i != 0 && i % pagesize == 0) ? 1
                                : 0) {
            Object ret = null;
