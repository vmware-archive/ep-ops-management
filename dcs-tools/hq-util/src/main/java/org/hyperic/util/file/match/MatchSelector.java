...
public class MatchSelector implements FileSelector {
    ...
    /**
     * If case-sensitive comparisons are off, this is Windows system so substitute unix-style path-separators with
     * Windows-style separators (i.e. we replace / with \)
     */
    ...
    public boolean isSelected(File basedir,
        ...
        for (i = 0; i < size; i++) {
            if (SelectorUtils.matchPath(excludePatterns.get(i).toString(),
                        fullpath, !isWindows)) {
                // log.info("MatchSelector(" + key + ") returning false because path (" + fullpath +
                // ") was an excluded path.");
                return false;
            }
        }
        ...
        for (i = 0; i < size; i++) {
            // log.info("MatchSelector(" + this + ") testing path=" + fullpath + " against pattern=" +
            // matchPatterns.get(i));
            if (SelectorUtils.matchPath(StringUtil.normalizePath(matchPatterns.get(i).toString().toLowerCase()),
                ...
                // log.info("MatchSelector(" + this + ") returning TRUE because path (" + fullpath +
                // ") MATCHED PATTERN=" + matchPatterns.get(i));
                return true;
            }
        }
