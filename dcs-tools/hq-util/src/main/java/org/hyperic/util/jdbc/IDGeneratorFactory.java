...
/**
 * This factory is meant to be the central point of access for IDGenerator objects used by an application. It will hold
 * the single IDGenerator in a private map and reuse it across repeated invocations of getNextId(). It is particularly
 * useful for Entity bean scenarios, where multiple instances of an IDGenerator result in database row contention and
 * SQL timeouts if more than one thread attempts to get a new id from the sequence at the same time.
 */
public class IDGeneratorFactory {
     ...
     * 
     * @param ctx - the logging context. Typically the name of the calling class
    ...
    private static IDGenerator getGenerator(String ctx,
                                            String sequenceName,
                                            DBUtil dbUtil)
        throws ConfigPropertyException,
        SequenceRetrievalException,
