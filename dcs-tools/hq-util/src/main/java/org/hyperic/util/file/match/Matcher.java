...
public class Matcher {
    ...
    private List initScanners(MatcherConfig config,
        ...
        masterSelectorArray[0] = new MasterMatchSelector(selectors,
    ...
    /**
     * Determine if the dir is on a filesystem that we should be searching.
     * 
     ...
     * @return true if the directory should be included in the search, false otherwise.
     */
    ...
    /**
     * Determine if the given filesystem is OK to search, based on the value of fstype which tells us what filesystem
     * types we are supposed to search.
     */
     ...
     * 
     * @return A Map representing the matches. The keys in the Map are the keys that each MatchSelector in the
     *         MatcherConfig was initialized with in its constructor. The values are Lists, where each element in the
     *         List is a String representing the full path of the matched path.
     * @exception MatcherInterruptedException If the search was interrupted before it could be completed. In this case,
     *                you can get the matches so far by calling getMatchesSoFar on the MatcherInterruptedException
     *                object.
     * @exception SigarException If an error occurs reading the available filesystems - this can only happen if the
     *                config's getFSTypes returns a value other than MatcherConfig.FS_ALL.
     */
     ...
     * 
     * @return true if dir is null, if dir is not a directory, if we can't read dir, or if dir does not exist.
     */
