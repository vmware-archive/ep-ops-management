...
public class DnsNameExtractor {
    ...
    public Collection<String> getDnsNames(final String url)
        throws Exception {
    ...
    private Certificate[] getCertificates(String url)
        throws Exception {
        try {
     ...
     */
    private Collection<String> getDnsNames(X509Certificate cert)
        ...
        List alternativeNames : x509Certificate.getSubjectAlternativeNames()) {
            Integer altNameType = (Integer) (alternativeNames.get(0));
            if (altNameType == DNS_NAME) {
                String aName = StringUtils.EMPTY + alternativeNames.get(1);
    ...
    private String getCn(String subject) {
        ...
        for (String subjectToken : subjectTokens) {
            if (!StringUtils.isEmpty(subjectToken) && subjectToken.startsWith("CN=")) {
