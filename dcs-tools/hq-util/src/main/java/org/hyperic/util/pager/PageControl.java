...
/**
 * A utility class to wrap up all the paging/sorting options that are frequently used with finders and other methods
 * that return lists of things.
 */
public class PageControl implements Serializable, Cloneable {
    ...
    /**
     * The SQL string representation of the above sort constants Note that the indices into this array line up with the
     * constant values above for the SORT_XXX constants.
     */
    public static final String[] SQL_SORTS =
    { "", "ASC", "DESC" };

    public static final PageControl PAGE_ALL = new PageControl(0, SIZE_UNLIMITED, true);
    ...
    private PageControl(int pagenum,
                        int pagesize,
                        boolean immutable) {
    ...
    public boolean isAscending() {
        return _sortorder == SORT_ASC;
    }

    public boolean isDescending() {
        return _sortorder == SORT_DESC;
    }

    /**
     * sets the initial defaults for the PageControl. Sort attribute specifies which attribute to sort on.
     * 
     * @param pc
