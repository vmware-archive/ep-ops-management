 ...
 * 
 * - Literal strings are required literal strings. e.g. '-value' means the string '-value' must exist
 * 
 * - '|' indicates that a <> container should accept the first value which matches any of the values. (I.e. <foo | bar>)
 * 
 * - '#' indicates a class to be called to handle the associated value
 * 
 * - Atoms must be surrounded by either '<>' or '[]' indicating the atom is required or optional, respectively. If the
 * atom contains a '$String' at the beginning, and if a BlockHandler is registered, it will be called after that block
 * is processed.
 * 
 * Example:
 * 
 * <$setVal -value #Integer> [<-from #PastDate> <-to #FutureDate> [-interval #com.mything.Interval]]
 */
            ...
            implements BlockHandler
{
     ...
     * 
     * @throws FormatException indicating there is an error in the formatText
     */
    public ParamParser(String formatText,
     ...
     * 
     * @param formatText Format to use when parsing parameters
     * @param retriever Object to call when attempting to instantiate ParserFormat objects to fulfill #class parameters
     * @param blockHandler Handler to use to process data blocks
     * 
     * @throws FormatException indicating there is an error in the formatText
     */
    ...
    private void init(ParserRetriever retriever,
                      String formatText,
                      BlockHandler blockHandler)
    {
        char[] formatChars;
    ...
    /**
     * Parse a parameter string. The validation of the parameter string will be performed, as well as calling of any
     * #class based format parsers.
     * 
     * @param params Parameters to validate against the format
     * 
     * @return An array of FormatParser[] objects which represent the output from the parsing process. StringParser
     *         objects are returned for every literal element in the parse string, else the parser types are defined by
     *         the #class elements in the format string
     * 
     ...
     */
    private int parseAtom(ParseResult res,
                          List procList,
                          FormatAtom atom,
                          String[] params,
                          int begIdx)
    ...
    private void dumpAtom(TextIndenter tInd,
                          FormatAtom atom,
                          String txt) {
    ...
    private void dumpFormat(TextIndenter tInd,
                            ContainerAtom atom) {
        String containerStr;
     ...
     */
    private int findClassNameEnd(char[] chars,
                                 int beginIdx,
                                 int endIdx) {
     ...
     */
    private int findLiteralEnd(char[] chars,
                               int beginIdx,
                               int endIdx) {
        for (int i = beginIdx; i < endIdx; i++) {
    ...
    private int findMatchingEnd(char[] chars,
                                char startToken,
                                char endToken,
                                int beginIdx,
                                int endIdx)
    ...
    private int findBlockNameEnd(char[] chars,
                                 int beginIdx,
                                 int endIdx) {
        for (int i = beginIdx; i < endIdx; i++) {
    ...
    /**
     * Parse the characters in 'formatChars' from start to end - 1, and return the Atom which the characters represent.
     */
    private ContainerAtom parseFormat(char[] formatChars,
                                      int start,
                                      int end) {
        ContainerAtom res;
        int i;
    ...
    private void callBlockHandler(ParseResult result,
                                  List blockData) {
