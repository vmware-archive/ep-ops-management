...
public class AutoApproveConfigTest {
    ...
    public static void setup()
        ...
        if (encKeyFile.exists()) {
            ...
            if (!deleted) {
                TestCase.fail("failed to delete encryption key file.");
            }
        }
        boolean created = encKeyFile.createNewFile();
        if (!created) {
            TestCase.fail("failed to create encryption key file.");
        }

        PropertyUtil.storeProperties(ENCRYPTION_KEY_FILE_NAME, new HashMap<String, String>() {
            {
                put("k",
                            "ENC(MHi1LeIpjDZxebFCEsHkLl6TGUBzq5ZfPcZ15ZFqS1apC+taJSjUdSsI+a5UHMdsqvB/8FVHxbIaNXfaGDDWVw\\=\\=)");
            }
        });
        ...
        if (propsFile.exists()) {
            ...
            if (!deleted) {
                TestCase.fail("failed to delete auto-approve properties file.");
            }
        }
        created = propsFile.createNewFile();
        if (!created) {
            TestCase.fail("failed to create auto-approve properties file.");
        }
    ...
    private void testCtorException(String agentConfigFolderName,
                                   String msg) {
