...
/**
 * Extends AIServerValue by allowing it to hold an array of AIServiceValue objects. Also provides a "placeholder" flag,
 * used to indicate to the server-side of runtime-AI that the object exists solely to carry services underneath it, and
 * that its other properties should not be used to update the corresponding appdef server.
 */
public class AIServerExtValue extends AIServerValue {
    ...
    /**
     * When true tells the HQ server that this server resource is ready to have metrics and runtime auto-inventory
     * enabled. This is in addition to have the metric ConfigResponse set and allows plugins to turn off AutoEnable when
     * there is more than 1 server resourcs with the same metric configuration.
     */
    ...
    /**
     * The hashCode of metric configuration values used to connect to this server for monitoring. When set makes it
     * possible to determine if two servers are being reported with the same config, in which case the server will only
     * auto enable metrics and runtime AI for the first server reported.
     */
