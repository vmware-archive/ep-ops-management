            ...
            implements IDisplayDataPoint, IComparableDatapoint, Serializable
{
    ...
    /**
     * Metric template is valid, but no value is available. Plugins may return this value, for example, where certain
     * metrics are only available depending on resource configuration, version, etc. NONE.getValue() is the VALUE_NONE
     * constant. This value will not be sent back to the server.
     */
    ...
    /**
     * Metric template is valid, but no value is available. The value is being collected in the background and is
     * expected to be available in the near future. The agent may re-try collection of measurements returning this value
     * sooner than the configured collection interval. FUTURE.getValue() is the VALUE_FUTURE constant. This value will
    ...
    /**
     * @return "None" if isNone(), "Future" if isFuture(), else NumberFormat.format(getValue())
     */
