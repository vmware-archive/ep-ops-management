...
/**
 * The result object from the GetMeasurements command. This is a tricky object, since it can potentially represent 2
 * different types of values: MeasurementValues and Exceptions. Since GetMeasurements can retrieve multiple measurements
 * at once, any one may fail, and therefore the caller should know if an exception occurred while retrieving it. The
 * caller should invoke obj.getException() if obj.getMeasurement() returns null, to get the exception message.
 */
...
public class GetMeasurements_result extends AgentRemoteValue {
    ...
    private Double getValue(int measNum,
                            String type) {
        try {
