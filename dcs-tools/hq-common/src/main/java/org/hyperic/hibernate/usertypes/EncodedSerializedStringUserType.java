...
/**
 * Hibernate custom {@link String} user type responsible for the serialization&encoding of objects into Base64
 * representation during an update/insert operation and the<br>
 * decoding & deserialization during select operations.
 * 
 * @author guy
 * 
 * @param <T> Proxied class type
 */
public class EncodedSerializedStringUserType<T> implements UserType {
    ...
    /**
     * 
     * @param clsReturnType Proxied class type to be used in actual accessors and mutatros.
     */
    ...
    /**
     * Extracts the String from the resultset using the names[0] value, Decodes from Base64 and finally deserializes.
     * 
     * @see ObjectCoder#decode(String, Class)
     * @param rs live & populated {@link ResultSet}
     * @param names [0] holds the column name
     ...
     * 
     * @param st {@link PreparedStatement} initialized with the insert/update query
     * @param value the Object to convert to encoded serialized string
     * @param index binding param index
     */
