...
/**
 * 
 * A hilo <tt>IdentifierGenerator</tt> that returns a <tt>Long</tt>, constructed using a hi/lo algorithm. The hi value
 * MUST be fetched in a seperate transaction to the <tt>Session</tt> transaction so the generator must be able to obtain
 * a new connection and commit it. If running in a Spring-managed transaction, the TransactionTemplate will be used to
 * suspend the existing transaction and create a new one. Else, an attempt will be made to open a new JDBC connection to
 * do the work.
 * 
 * A hilo <tt>IdentifierGenerator</tt> that uses a database table to store the last generated values. A table can
 * contains several hi values. They are distinct from each other through a key
 * <p/>
 * <p>
...
public class HQMultipleHiLoPerTableGenerator
    ...
    private Integer updateSequence(Connection connection)
        throws SQLException {
        int result;
        int rows;
        do {
    ...
    private synchronized Number _generate(final SessionImplementor session,
                                          Object obj)
        throws HibernateException {
