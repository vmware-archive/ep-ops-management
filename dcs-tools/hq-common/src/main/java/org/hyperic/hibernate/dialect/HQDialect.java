...
public interface HQDialect
{
     ...
     * @param cost - based on the database, may be table percentage or random number
     */
    ...
    /**
     * Returns true if using the EAM_NUMBERS table to iterate over a certain amount of time ranges is optimal for the
     * database
     */
    public boolean useEamNumbers();

    /**
     * Returns -1 if Max Expressions supported in the db is unlimited. This applies mainly to SQL in statements e.g.
     * where ids in (0, 1, 2,...) Or a sequence of SQL 'and' or 'or' statements in one statement
     */
     ...
     * @param regex - the POSIX style regex. The param passed in allows for prepared statement type syntax. If that is
     *            not desired put quotes around the value to ensure the SQL will not fail.
