...
/**
 * DBPing is a tool for testing that a given JDBC driver can be loaded an a connection to a database established. In
 * addition, DBPing can run a simple performance test which does the following: 1. Creates a table 2. Inserts records
 * (1000 records by default; this can be overriden) 3. Commits 4. Deletes the records 5. Commits 6. Drops the table
 * 
 * DBPing can be run from the command line using the command: java org.hyperic.tools.db.DBPing Your JDBC driver should
 * be in your classpath, as well as the DBPing class.
 * 
 * DBPing can also be called directly from another class. To do this instantiate the DBPing class and then call the
 * DBPing.ping() method. The DBPing.speedTest() method call also be used to run a performance test.
 */
public class DBPing
{
    ...
    /**
     * Runs an operation test by creating a table, creating an index, creating a sequence, creating a trigger, inserting
     * a row of data, updating a row of data and deleting a row of data. All of the created objects are dropped from the
     * database before the method completes.
     * 
     * @param database The JDBC driver connect string (e.g., "jdbc:cloudscape:test")
     */
    public void operationTest(String database)
     ...
     * database before the method completes.
     * 
     * @param driver The Java class name of the JDBC driver (e.g., "COM.cloudscape.core.JDBCDriver"). This paramater can
     *            be null in which case the proper driver is loaded automatically, based on the specified database.
     * @param database The JDBC driver connect string (e.g., "jdbc:cloudscape:test")
     */
    public void operationTest(String driver,
                              String database)
        throws ClassNotFoundException, SQLException
    {
     ...
     *            be null in which case the proper driver is loaded automatically, based on the specified database.
     * @param database The JDBC driver connect string (e.g., "jdbc:cloudscape:test")
     * @param username The username to use when connecting to the database. This value can be null.
     * @param password The password to use when connecting to the database. This value can be null.
     */
    public void operationTest(String driver,
                              String database,
                              String username,
     ...
     * @param database The JDBC driver connect string (e.g., "jdbc:cloudscape:test")
     */
    public void ping(String database)
        throws ClassNotFoundException, SQLException
    {
     ...
     * 
     * @param database The JDBC driver connect string (e.g., "jdbc:cloudscape:test")
     * @param driver The Java class name of the JDBC driver (e.g., "COM.cloudscape.core.JDBCDriver").
     */
    public void ping(String driver,
                     String database)
        throws ClassNotFoundException, SQLException
    {
    ...
    /**
     * Runs a speed test by creating a table, inserting a specified number of rows into a table, committing, deleting
     * the rows committing and then dropping the table. The created table starts with the phrase DBPING_ and ends with a
     * random number.
     * 
     * @param database The JDBC driver connect string (e.g., "jdbc:cloudscape:test")
     * @param username The username to use when connecting to the database. This value can be null.
     * @param password The password to use when connecting to the database. This value can be null.
     * @param numrecs The number of records to create and delete.
     * 
     * @return long The elapsed time in milliseconds that the speed test required.
     */
    public long speedTest(String driver,
                          String database,
                          String username,
                          String password)
        throws ClassNotFoundException, SQLException
        ...
        throws ClassNotFoundException, SQLException
    {
        ...
        try
        {
            ...
            strCmd =
                        "CREATE TABLE "
                                    + strTableName
                                    + " (ID INT PRIMARY KEY, NAME CHAR(30), ADDRESS1 CHAR(30), CITY CHAR(30), STATE CHAR(2), ZIP CHAR(5))";
            stmt.executeUpdate(strCmd);
            conn.commit();
            bDropTable = true;
            ...
            for (int i = 1; i <= numrecs; i++)
            {
                ...
                stmt.executeUpdate("INSERT INTO " + strTableName + " VALUES(" + i
                            + ", 'Mark Douglas', '645 Howard Street', 'San Francisco', 'CA', '94105')");
            }
    ...
    /**
     * Runs a multi-threaded speed test by creating a table, inserting a specified number of rows into a table,
     * committing, deleting the rows committing and then dropping the table. The created table starts with the phrase
     * DBPING_ and ends with a random number.
     * 
     * @param database The JDBC driver connect string (e.g., "jdbc:cloudscape:test")
     * @param username The username to use when connecting to the database. This value can be null.
     * @param password The password to use when connecting to the database. This value can be null.
     * @param numrecs The number of records to create and delete.
     * @param threads The number of threads to use in the speed test.
     * 
     * @return long The elapsed time in milliseconds that the speed test required.
     */
    public long speedTest(String driver,
            ...
            try
            {
                ...
                for (int i = 0; i < threads; i++)
                {
                    SpeedTestThread thread =
                                new SpeedTestThread(database, username, password, strTableName,
                                            (iRecsPerThread * i) + 1, (iRecsPerThread) * (i + 1));
