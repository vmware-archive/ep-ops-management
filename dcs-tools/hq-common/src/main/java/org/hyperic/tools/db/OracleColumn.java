...
{
    protected OracleColumn(Node node,
                           Table table)
        throws SAXException
    {
        super(node, table);
        ...
        if (this.hasDefault() == true)
        {
            switch (this.getDefault()) {
                ...
                case Column.DEFAULT_SEQUENCE_ONLY:
                    ...
                    cmds.add(0,
                                "CREATE SEQUENCE " + this.m_strTableName + '_' + this.getName() + "_SEQ"
                                            + " START WITH " + this.getInitialSequence() + " INCREMENT BY "
                                            + this.getIncrementSequence() + " NOMAXVALUE NOCYCLE CACHE 10");
                    break;
            }
        }
