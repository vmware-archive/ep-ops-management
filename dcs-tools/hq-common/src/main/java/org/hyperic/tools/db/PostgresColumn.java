...
{
    protected PostgresColumn(Node node,
                             Table table)
        throws SAXException
    {
        super(node, table);
    ...
    protected String getDefaultCommand(List cmds)
    {
        if (m_strType.equalsIgnoreCase("auto") ||
                    m_iDefault == Column.DEFAULT_SEQUENCE_ONLY)
            return "";
        ...
        if (hasDefault()) {
            switch (getDefault()) {
                ...
                case Column.DEFAULT_SEQUENCE_ONLY:
                    cmds.add(0, "CREATE SEQUENCE " + this.m_strTableName + '_' + this.getName() + "_SEQ" + " START "
                                + this.getInitialSequence() + " INCREMENT " + this.getIncrementSequence() + " CACHE 10");
                    break;
            }
        }
    }
