...
public class PostgreSQL {
    private static final String PASSWORD_AUTHENTICATION_FAILED_MSG = "FATAL: password authentication failed";
    protected static final String PROP_DATABASE = "Database";
    ...
    /**
     * SQL Exceptions can expose sensitive information. Therefore, sanitizing them and returning a safe PluginException
     * instead. Exception trace will be lost if contains sensitive information.
     */
    public static PluginException getSecuredException(SQLException e) {
        if (e.getMessage() != null && e.getMessage().startsWith(PASSWORD_AUTHENTICATION_FAILED_MSG)) {
            return new PluginException("Error querying for Databases: " + PASSWORD_AUTHENTICATION_FAILED_MSG);
        }
        return new PluginException("Error querying for Databases: " + e.getMessage(), e);
    }
}
