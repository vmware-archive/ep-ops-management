...
public class SystemPlugin extends ProductPlugin {
    ...
    public GenericPlugin aglyGetPlugin(String type,
                                       TypeInfo info) {
    ...
    private void addWindowsService(TypeBuilder types) {
        ...
        service.setModel(getModel(SVC_NAME));
    ...
    private void addProcessServices(TypeBuilder types) {
        ...
        for (int i = 0; i < PROCESS_SERVICES.length; i++) {
            ...
            service.setModel(getModel(name));
            types.add(service);
        }
    }
    ...
    public TypeInfo[] getTypes() {
        ...
        for (int i = 0; i < FILE_SERVICES.length; i++) {
            ...
            types.addService(server, name, getModel(name));
        }

        ServiceTypeInfo script =
                    new ServiceTypeInfo(SCRIPT_NAME, SCRIPT_NAME, server);
        script.setModel(getModel(SCRIPT_NAME));
        ...
        switch (info.getType()) {
            ...
            case TypeInfo.TYPE_SERVICE:
                            ...
                            info.isService(DIR_TREE_NAME))
                {
                    ...
                    ConfigOption option = schema.add(SystemMeasurementPlugin.PTQL_CONFIG,
                                "Multi-process query. For example: State.Name.eq=httpd",
                                "State.Name.eq=");
                    ...
                    schema.add("prefix",
                                ...
                                "").setOptional(true);
                    ConfigOption option = schema.add(PROP_PATH,
    ...
    /**
     * This plugin has to be loaded first. Almost all other plugins require the OS platforms to be parents of their
     * server types
     */
    ...
    private ObjectModel getModel(String serviceName) {
        if (standardServicesList.contains(serviceName)) {
