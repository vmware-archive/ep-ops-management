...
public class FileSystemDetector
    ...
    protected ArrayList getSystemServiceValues(Sigar sigar,
        ...
        if (isWin32()) {
            ...
            for (String disk : disks) {
                if (!disk.equalsIgnoreCase("_total")) {
                    AIServiceValue svc =
                                createSystemService(SystemPlugin.PHYSICAL_DISK_SERVICE, getFullServiceName(disk),
                                            "name", disk);
                    services.add(svc);
                }
            }
            ...
            for (String disk : disks) {
                if (!disk.equalsIgnoreCase("_total")) {
                    AIServiceValue svc =
                                createSystemService(SystemPlugin.LOGICAL_DISK_SERVICE, getFullServiceName(disk),
                                            "name", disk);
                    services.add(svc);
                }
            }
            ...
            for (String dev : devices) {
                ...
                AIServiceValue svc =
                            createSystemService(SystemPlugin.BLOCK_DEVICE_SERVICE, getFullServiceName("/dev/" + dev),
                                        "name", dev);
                services.add(svc);
            }
        }
        ...
        for (int i = 0; i < fslist.length; i++) {
            ...
            String info = fs.getDevName() + mntInfo + " (" + genericType + "/" + type + ")";
    ...
    private List getFileServiceImports(Sigar sigar,
                                       String type) {
    ...
    private void addFileServices(List services,
                                 boolean recurse,
                                 File dir,
                                 Sigar sigar,
                                 String type) {
    ...
    private List getFileServices(Sigar sigar,
                                 String type) {
    ...
    private void setFileProperties(Sigar sigar,
                                   AIServiceValue svc,
                                   String file) {
