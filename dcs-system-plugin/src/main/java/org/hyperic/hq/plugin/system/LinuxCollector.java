...
public class LinuxCollector extends Collector {
    ...
    public void collect() {
        ...
        for (String[] fields : stats) {
            ...
            double rd_ios = Double.parseDouble(fields[LinuxUtils.rd_ios]);
            double wr_ios = Double.parseDouble(fields[LinuxUtils.wr_ios]);
            double tot_ticks = Double.parseDouble(fields[LinuxUtils.tot_ticks]);
            double rq_ticks = Double.parseDouble(fields[LinuxUtils.rq_ticks]);

            sdc.nr_ios = rd_ios + wr_ios;
            sdc.rd_sect = Double.parseDouble(fields[LinuxUtils.rd_sect]);
            sdc.wr_sect = Double.parseDouble(fields[LinuxUtils.wr_sect]);
            sdc.rd_ticks = Double.parseDouble(fields[LinuxUtils.rd_ticks]);
            sdc.wr_ticks = Double.parseDouble(fields[LinuxUtils.wr_ticks]);
